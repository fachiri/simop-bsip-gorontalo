...
@section('title', 'Tambah PJ')
@section('content')
	<section class="row">
		<div class="col-12">
			<div class="card">
				<div class="card-header d-flex justify-content-between align-items-center">
					<h4 class="card-title pl-1">Form Tambah PJ</h4>
				</div>
				<div class="card-body px-4">
					<x-form.layout.horizontal action="{{ route('dashboard.master.pics.store') }}" method="POST">
						<x-form.input layout="horizontal" name="name" label="Nama Lengkap" placeholder="Nama Lengkap.." />
						<x-form.select layout="horizontal" name="department" label="Bidang" :options="$departments->map(function ($department) {
						    return (object) [
						        'label' => $department,
						        'value' => $department,
						    ];
